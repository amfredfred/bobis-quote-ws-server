...
export function calculateLotSize(params: {
  ...
  const { accountBalance, riskMode, riskPercent, riskFixed,
          entryPrice, stopLoss, symbolInfo, maxLot, minLot } = params;
  ...
  logger.debug('lot calc result', {
    symbol: symbolInfo.symbol, riskMode,
    riskAmount: riskAmount.toFixed(2), riskPips: riskPips.toFixed(1),
    pipValue: pipValue.toFixed(6), rawLots: rawLots.toFixed(4), lotSize,
  });
